#!/usr/bin/env python3
"""
Batch Google Drive Cleanup

This script processes files in small batches to avoid timeouts.
"""
...
def batch_cleanup(batch_size=20):
    """Process files in small batches."""

    backup = GoogleDriveBackup()
    folder_id = '157I1VHclMJETmfE_nEt5bsS0_4Bhv7kA'

    logger.info(f"Starting batch cleanup (batch size: {batch_size})")
    ...
    try:
        ...
        for i in range(0, len(all_files), batch_size):
            ...
            logger.info(f"Processing batch {i//batch_size + 1}: files {i+1}-{min(i+batch_size, len(all_files))}")
...
if __name__ == "__main__":
    print("Batch Google Drive Cleanup")
    print("=" * 50)

    kept, removed = batch_cleanup(batch_size=10)  # Small batches
