#!/usr/bin/env python3
"""
Danger/Safety Section Word Count Plugin - Enforces word count for Danger/Safety section.

Validates that the Danger/Safety section has optimal word count based on CatBoost analysis.
...
class DangerSafetyWordCountPlugin(BaseFeaturePlugin):
    """Plugin to validate word count for Danger/Safety section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "danger_safety_word_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("DangerSafetyWordCountPlugin initialized")

        # Load optimal word count range from analysis data
        loader = get_loader()
        word_count_range = loader.get_word_count_range('danger_safety')
        ...
        if word_count_range:
            ...
            self.optimal_min = targets.get("min_word_count", 497)
            self.optimal_max = targets.get("max_word_count", 2752)
    ...
    def _analyze_section_word_count(self, section_text: str) -> Dict[str, Any]:
        """Analyze word count in Danger/Safety section."""
        if not section_text:
            return {
                "word_count": 0,
                "meets_target": False,
    ...
    async def validate_danger_safety_word_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """
        Validate word count for Danger/Safety section.
        """
            ...
            return FunctionResult(
                ...
                data={
                    ...
                    "issues": [{
                        ...
                        "severity": "high",
                        "message": "Danger/Safety section not found",
                        "suggestion": "Add a Danger/Safety section"
                    }]
                },
                message="Danger/Safety section not found"
            )
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for danger_safety word_count: {e}")
        ...
        if not analysis["meets_target"]:
            if analysis["word_count"] < self.optimal_min:
                issues.append({
                    ...
                    "severity": "medium",
                    "message": f"Danger/Safety section word count ({analysis['word_count']}) is below optimal range ({self.optimal_min}-{self.optimal_max} words, Q25-Q75 from successful motions)",
                    "suggestion": f"Increase Danger/Safety section to at least {self.optimal_min} words"
                })
                recommendation = {
                    "type": "increase_section_length",
                    "priority": "medium",
                    "action": f"Increase Danger/Safety section from {analysis['word_count']} to at least {self.optimal_min} words",
                    "rationale": "Optimal word count range for Danger/Safety section"
                }
                ...
                issues.append({
                    ...
                    "severity": "low",
                    "message": f"Danger/Safety section word count ({analysis['word_count']}) exceeds optimal range ({self.optimal_min}-{self.optimal_max})",
                    "suggestion": f"Consider condensing Danger/Safety section to {self.optimal_max} words or less"
                })
        else:
            recommendations.append({
                "type": "section_length_optimal",
                "message": f"Danger/Safety section word count is optimal ({analysis['word_count']} words, range: {self.optimal_min}-{self.optimal_max})"
            })
        ...
        return FunctionResult(
            ...
            message="Danger/Safety section word count validation complete"
        )
