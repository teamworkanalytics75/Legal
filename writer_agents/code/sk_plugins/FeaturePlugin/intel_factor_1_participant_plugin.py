#!/usr/bin/env python3
"""
Enforces Intel Factor 1: participant status analysis

Intel Corp. v. Advanced Micro Devices, Inc., 542 U.S. 241, 264 (2004)
Key cases: Gorsoan Ltd. v. Bullock, 652 F. App'x 44 (2d Cir. 2016), Pishevar v. Fusion GPS, 2017 WL 1184305 (D.D.C. 2017)
"""
...
class IntelFactor1ParticipantPlugin(BaseFeaturePlugin):
    """Plugin: Enforces Intel Factor 1: participant status analysis"""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "intel_factor_1_participant", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("IntelFactor1ParticipantPlugin initialized")

    async def validate_intel_factor_1_participant(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """
        Validate intel_factor_1_participant in document.
        ...
        checklist_items = ['Discovery target identified as non-participant', 'Evidence that target is NOT a party to foreign proceeding', 'If target IS a party, justification for Section 1782 aid', 'Separate legal entities distinguished (if applicable)']
        ...
        required_phrases = [['If', 'target', 'IS'], ['Separate', 'legal', 'entities']]
        ...
        return FunctionResult(
            ...
            metadata={
                "feature_name": "intel_factor_1_participant",
                "priority": "high",
                "threshold": 0.9,
                "legal_standard": "Intel Corp. v. Advanced Micro Devices, Inc., 542 U.S. 241, 264 (2004)"
            }
        )
