#!/usr/bin/env python3
"""
Comprehensive 1782 PDF Downloader - Downloads ALL PDFs including attachments and exhibits
"""
...
class Comprehensive1782PDFDownloader:
    def __init__(self, config_path: str = "document_ingestion/courtlistener_config.json"):
        ...
        self.pdf_dir = Path("data/case_law/1782_comprehensive_pdfs")
        ...
        logger.info("Comprehensive 1782 PDF Downloader initialized")
    ...
    def search_1782_cases_comprehensive(self, max_results: int = 100) -> List[Dict[str, Any]]:
        """Search CourtListener comprehensively for 1782 discovery cases."""
        logger.info(f"🔍 Comprehensive search for 1782 discovery cases (max {max_results})")
    ...
    def get_all_pdfs_from_case(self, case_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Get ALL PDFs from a case including opinions, docket entries, and attachments."""
        case_name = case_data.get('caseName', 'Unknown')
        cluster_id = case_data.get('cluster_id')
        docket_id = case_data.get('docket_id')

        logger.info(f"📄 Getting ALL PDFs from case: {case_name}")
        logger.info(f"   Cluster ID: {cluster_id}, Docket ID: {docket_id}")

        all_pdfs = []

        # 1. Get opinions from cluster
        ...
        # 2. Get docket entries and their attachments
        if docket_id:
            logger.info(f"Getting docket entries for docket {docket_id}")
            docket_pdfs = self.get_docket_pdfs_comprehensive(docket_id)
            all_pdfs.extend(docket_pdfs)

        logger.info(f"✓ Found {len(all_pdfs)} total PDFs for case: {case_name}")
        return all_pdfs

    def get_docket_pdfs_comprehensive(self, docket_id: str) -> List[Dict[str, Any]]:
        """Get ALL PDFs from docket entries including attachments."""
        logger.info(f"📋 Getting comprehensive docket PDFs for docket {docket_id}")

        # Get docket entries
        entries_response = self.make_api_request(f'/dockets/{docket_id}/docket-entries/')
        if not entries_response or 'results' not in entries_response:
            logger.warning(f"Could not retrieve docket entries for docket {docket_id}")
            return []
        ...
        for i, entry in enumerate(entries):
            ...
            for j, doc in enumerate(recap_docs):
                ...
                if filepath and filepath.endswith('.pdf'):
                    ...
                    all_docket_pdfs.append(pdf_info)
                    self.stats['docket_entries_downloaded'] += 1
                    logger.info(f"  ✓ Added PDF: {filepath}")

        logger.info(f"✓ Found {len(all_docket_pdfs)} docket PDFs")
        return all_docket_pdfs
    ...
    def process_case_comprehensive(self, case_data: Dict[str, Any]):
        """Process a single case comprehensively."""
        case_name = case_data.get('caseName', 'Unknown')
        logger.info(f"\n📋 Processing case: {case_name}")

        try:
            # Get all PDFs from this case
            case_pdfs = self.get_all_pdfs_from_case(case_data)
    ...
    def run_comprehensive_download(self, max_cases: int = 50):
        ...
        logger.info("📄 Including: Opinions, Docket Entries, Attachments, Exhibits")
        logger.info(f"📁 PDFs will be saved to: {self.pdf_dir.absolute()}")

        # Search for 1782 cases comprehensively
        cases = self.search_1782_cases_comprehensive(max_results=max_cases)
        ...
        for i, case_data in enumerate(cases):
            ...
            self.process_case_comprehensive(case_data)
        ...
        logger.info(f"   Attachments downloaded: {self.stats['attachments_downloaded']}")
    ...
    def generate_summary_report(self):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**Attachments Downloaded**: {self.stats['attachments_downloaded']}\n")
            ...
            f.write("## Strategy\n")
            f.write("- **Comprehensive Search**: Multiple search strategies for 1782 cases\n")
            f.write("- **All PDF Types**: Downloads opinions, docket entries, attachments, exhibits\n")
            f.write("- **RECAP Integration**: Accesses docket entries and their attachments\n")
            f.write("- **Complete Coverage**: Gets everything available for each case\n\n")
...
def main():
    ...
    downloader.run_comprehensive_download(max_cases=args.max_cases)
