#!/usr/bin/env python3
"""
Factual Background Section Words Per Sentence Plugin - Enforces words per sentence for Factual Background section.

Validates that sentences in the Factual Background section have optimal word counts based on analysis.
"""
...
class FactualBackgroundWordsPerSentencePlugin(BaseFeaturePlugin):
    """Plugin to validate words per sentence for Factual Background section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "factual_background_words_per_sentence", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("FactualBackgroundWordsPerSentencePlugin initialized")

        loader = get_loader()
        words_range = loader.get_words_per_sentence_range('factual_background')
    ...
    async def validate_factual_background_words_per_sentence(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate words per sentence for Factual Background section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for factual_background words_per_sentence: {e}")
        
        if not analysis["meets_target"]:
            issues.append({
                "type": "suboptimal_words_per_sentence",
                "severity": "low",
                "message": f"Factual Background section has {analysis['sentences_in_range']}/{analysis['total_sentences']} sentences with optimal word count ({self.optimal_min}-{self.optimal_max} words)",
            ...
            recommendation = {
                ...
                "rationale": "Optimal words per sentence for Factual Background section"
            }
            ...
            recommendations.append({
                ...
                "message": f"Factual Background section words per sentence is optimal (avg: {analysis['avg_words_per_sentence']:.1f})"
            })
        ...
        return FunctionResult(
            ...
            message="Factual Background section words per sentence validation complete"
        )
