#!/usr/bin/env python3
"""
Protective Measures Section Sentence Count Plugin - Enforces sentences per paragraph for Protective Measures section.

Validates that paragraphs in the Protective Measures section have optimal sentence counts based on analysis.
"""
...
class ProtectiveMeasuresSentenceCountPlugin(BaseFeaturePlugin):
    """Plugin to validate sentences per paragraph for Protective Measures section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "protective_measures_sentence_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("ProtectiveMeasuresSentenceCountPlugin initialized")

        loader = get_loader()
        sentences_range = loader.get_sentences_per_paragraph_range('protective_measures')
    ...
    async def validate_protective_measures_sentence_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate sentences per paragraph for Protective Measures section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for protective_measures sentence_count: {e}")
        
        if not analysis["meets_target"]:
            issues.append({
                "type": "suboptimal_sentence_count",
                "severity": "low",
                "message": f"Protective Measures section has {analysis['paragraphs_in_range']}/{analysis['total_paragraphs']} paragraphs with optimal sentence count ({self.optimal_min}-{self.optimal_max} sentences)",
            ...
            recommendation = {
                ...
                "rationale": "Optimal sentence count per paragraph for Protective Measures section"
            }
            ...
            recommendations.append({
                ...
                "message": f"Protective Measures section sentence count per paragraph is optimal (avg: {analysis['avg_sentences_per_paragraph']:.1f})"
            })
        ...
        return FunctionResult(
            ...
            message="Protective Measures section sentence count validation complete"
        )
