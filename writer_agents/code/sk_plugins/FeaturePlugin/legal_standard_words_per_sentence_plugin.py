#!/usr/bin/env python3
"""
Legal Standard Section Words Per Sentence Plugin - Enforces words per sentence for Legal Standard section.

Validates that sentences in the Legal Standard section have optimal word counts based on analysis.
"""
...
class LegalStandardWordsPerSentencePlugin(BaseFeaturePlugin):
    """Plugin to validate words per sentence for Legal Standard section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "legal_standard_words_per_sentence", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("LegalStandardWordsPerSentencePlugin initialized")

        loader = get_loader()
        words_range = loader.get_words_per_sentence_range('legal_standard')
    ...
    async def validate_legal_standard_words_per_sentence(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate words per sentence for Legal Standard section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for legal_standard words_per_sentence: {e}")
        
        if not analysis["meets_target"]:
            issues.append({
                "type": "suboptimal_words_per_sentence",
                "severity": "low",
                "message": f"Legal Standard section has {analysis['sentences_in_range']}/{analysis['total_sentences']} sentences with optimal word count ({self.optimal_min}-{self.optimal_max} words)",
            ...
            recommendation = {
                ...
                "rationale": "Optimal words per sentence for Legal Standard section"
            }
            ...
            recommendations.append({
                ...
                "message": f"Legal Standard section words per sentence is optimal (avg: {analysis['avg_words_per_sentence']:.1f})"
            })
        ...
        return FunctionResult(
            ...
            message="Legal Standard section words per sentence validation complete"
        )
