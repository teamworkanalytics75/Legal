#!/usr/bin/env python3
"""
Upload All 65 Unique §1782 Cases to Google Drive

Upload the complete collection of 65 unique §1782 cases to the clean Google Drive folder.
"""

import sys
import json
import logging
from pathlib import Path

# Add document_ingestion to path
sys.path.insert(0, str(Path(__file__).parent.parent))

# Import Google Drive backup module
import importlib.util
spec = importlib.util.spec_from_file_location(
    "google_drive_backup",
    Path(__file__).parent.parent / "document_ingestion" / "google_drive_backup.py"
)
gdrive_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(gdrive_module)
GoogleDriveBackup = gdrive_module.GoogleDriveBackup

# Setup logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
logger = logging.getLogger(__name__)


class CaseUploader:
    """Upload all unique §1782 cases to Google Drive."""

    def __init__(self):
        """Initialize the uploader."""
        self.gdrive = GoogleDriveBackup()
        self.data_dir = Path(__file__).parent.parent / "data" / "case_law"

        # Clean folder ID (the one we created earlier)
        self.clean_folder_id = "1_wv4FZLhubYOO2ncRBcW68w4htt4N5Pl"

    def load_all_unique_cases(self):
        """Load all unique cases from both collections."""
        try:
            all_cases = []

            # Load web-scraped cases
            web_cases_file = self.data_dir / "unique_web_cases.json"
            if web_cases_file.exists():
                with open(web_cases_file, 'r', encoding='utf-8') as f:
                    web_cases = json.load(f)
                all_cases.extend(web_cases)
                logger.info(f"Loaded {len(web_cases)} unique web cases")

            # Load previous cases
            prev_cases_file = self.data_dir / "unique_previous_cases.json"
            if prev_cases_file.exists():
                with open(prev_cases_file, 'r', encoding='utf-8') as f:
                    prev_cases = json.load(f)
                all_cases.extend(prev_cases)
                logger.info(f"Loaded {len(prev_cases)} unique previous cases")

            logger.info(f"Total unique cases to upload: {len(all_cases)}")
            return all_cases

        except Exception as e:
            logger.error(f"Error loading unique cases: {e}")
            return []

    def create_case_json(self, case):
        """Create a properly formatted JSON file for a case."""
        try:
            # Create a comprehensive case record
            case_record = {
                'case_id': case.get('case_id', ''),
                'case_name': case.get('case_name', ''),
                'court': case.get('court', ''),
                'docket_number': case.get('docket_number', ''),
                'date_filed': case.get('date_filed', ''),
                'status': case.get('status', ''),
                'citations': case.get('citations', ''),
                'case_url': case.get('case_url', ''),
                'case_text': case.get('case_text', ''),
                'metadata': case.get('metadata', ''),
                'source': case.get('source', 'unknown'),
                'upload_timestamp': case.get('upload_timestamp', ''),
                'analysis': case.get('analysis', {})
            }

            return json.dumps(case_record, indent=2, ensure_ascii=False)

        except Exception as e:
            logger.error(f"Error creating case JSON: {e}")
            return None

    def generate_filename(self, case):
        """Generate a clean filename for the case."""
        try:
            case_name = case.get('case_name', 'Unknown Case')
            case_id = case.get('case_id', '')

            # Clean up the case name
            filename = case_name.replace('Application of ', '')
            filename = filename.replace('Application for ', '')
            filename = filename.replace('In re ', '')
            filename = filename.replace('In the Matter of ', '')

            # Remove court info from filename (it's in metadata)
            filename = filename.split(' (')[0]

            # Clean up special characters
            filename = filename.replace(':', '')
            filename = filename.replace('/', '')
            filename = filename.replace('\\', '')
            filename = filename.replace('?', '')
            filename = filename.replace('*', '')
            filename = filename.replace('"', '')
            filename = filename.replace('<', '')
            filename = filename.replace('>', '')
            filename = filename.replace('|', '')

            # Limit length
            if len(filename) > 100:
                filename = filename[:100]

            # Add case ID if available
            if case_id:
                filename = f"{filename} (ID_{case_id})"

            # Ensure it ends with .json
            if not filename.endswith('.json'):
                filename += '.json'

            return filename

        except Exception as e:
            logger.error(f"Error generating filename: {e}")
            return f"case_{case.get('case_id', 'unknown')}.json"

    def upload_case_to_drive(self, case, folder_id):
        """Upload a single case to Google Drive."""
        try:
            # Create JSON content
            json_content = self.create_case_json(case)
            if not json_content:
                logger.error(f"Failed to create JSON for case: {case.get('case_name', 'Unknown')}")
                return False

            # Generate filename
            filename = self.generate_filename(case)

            # Upload to Google Drive
            file_metadata = {
                'name': filename,
                'parents': [folder_id]
            }

            # Create the file
            file = self.gdrive.service.files().create(
                body=file_metadata,
                media_body=json_content,
                fields='id, name'
            ).execute()

            logger.info(f"✅ Uploaded: {filename}")
            return True

        except Exception as e:
            logger.error(f"❌ Error uploading case: {e}")
            return False

    def upload_all_cases(self):
        """Upload all unique cases to Google Drive."""
        logger.info("Starting upload of all unique §1782 cases...")

        # Load all cases
        all_cases = self.load_all_unique_cases()
        if not all_cases:
            logger.error("No cases to upload")
            return False

        # Upload each case
        successful_uploads = 0
        failed_uploads = 0

        for i, case in enumerate(all_cases, 1):
            logger.info(f"Uploading case {i}/{len(all_cases)}: {case.get('case_name', 'Unknown')[:60]}...")

            success = self.upload_case_to_drive(case, self.clean_folder_id)
            if success:
                successful_uploads += 1
            else:
                failed_uploads += 1

        # Summary
        logger.info(f"\nUPLOAD SUMMARY:")
        logger.info(f"Total cases: {len(all_cases)}")
        logger.info(f"Successful uploads: {successful_uploads}")
        logger.info(f"Failed uploads: {failed_uploads}")
        logger.info(f"Success rate: {successful_uploads/len(all_cases)*100:.1f}%")

        return successful_uploads > 0

    def create_final_database_summary(self):
        """Create a final database summary."""
        try:
            # Load all cases
            all_cases = self.load_all_unique_cases()

            # Create summary
            summary = {
                'database_name': 'The Art of War - §1782 Caselaw Database',
                'total_cases': len(all_cases),
                'creation_date': '2025-10-15',
                'sources': {
                    'courtlistener_api': '11 cases',
                    'courtlistener_web_scraping': '54 cases'
                },
                'case_breakdown': {
                    'verified_1782_cases': len([c for c in all_cases if '1782' in c.get('case_name', '').lower()]),
                    'application_cases': len([c for c in all_cases if 'application' in c.get('case_name', '').lower()]),
                    'federal_cases': len([c for c in all_cases if any(court in c.get('court', '').lower() for court in ['cir', 'd.', 'fed'])]),
                    'recent_cases': len([c for c in all_cases if '202' in c.get('date_filed', '')])
                },
                'google_drive_folder': {
                    'name': 'The Art of War - Caselaw Database',
                    'id': self.clean_folder_id,
                    'url': f'https://drive.google.com/drive/folders/{self.clean_folder_id}'
                },
                'file_locations': {
                    'all_cases': str(self.data_dir / "merged_1782_cases.json"),
                    'web_cases': str(self.data_dir / "unique_web_cases.json"),
                    'previous_cases': str(self.data_dir / "unique_previous_cases.json"),
                    'analysis_summary': str(self.data_dir / "case_analysis_summary.json")
                }
            }

            # Save summary
            summary_path = self.data_dir / "final_database_summary.json"
            with open(summary_path, 'w', encoding='utf-8') as f:
                json.dump(summary, f, indent=2, ensure_ascii=False)

            logger.info(f"Final database summary saved: {summary_path}")

            return summary

        except Exception as e:
            logger.error(f"Error creating final database summary: {e}")
            return None


def main():
    """Main entry point."""
    uploader = CaseUploader()

    # Upload all cases
    success = uploader.upload_all_cases()

    if success:
        # Create final database summary
        summary = uploader.create_final_database_summary()

        print(f"\n🎉 UPLOAD COMPLETE!")
        print(f"All unique §1782 cases uploaded to Google Drive")
        print(f"Final database summary created")
        print(f"Google Drive folder: https://drive.google.com/drive/folders/{uploader.clean_folder_id}")
    else:
        print(f"\n❌ Upload failed")


if __name__ == "__main__":
    main()
