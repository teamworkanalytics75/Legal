#!/usr/bin/env python3
"""
Introduction Section Word Count Plugin - Enforces word count for Introduction section.
...
class IntroductionWordCountPlugin(BaseFeaturePlugin):
    """Plugin to validate word count for Introduction section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "introduction_word_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("IntroductionWordCountPlugin initialized")

        # Load optimal word count range from analysis data
        loader = get_loader()
        word_count_range = loader.get_word_count_range('introduction')
        ...
        if word_count_range:
            ...
            self.optimal_min = targets.get("min_word_count", 152)
            self.optimal_max = targets.get("max_word_count", 1326)
            logger.warning(f"Using fallback range for introduction.word_count: {self.optimal_min}-{self.optimal_max} (no optimal_range, optimal_threshold, or successful_mean found in analysis data)")

    def _analyze_section_word_count(self, section_text: str) -> Dict[str, Any]:
        """Analyze word count in Introduction section."""
        if not section_text:
            return {
                "word_count": 0,
                "meets_target": False,
    ...
    async def validate_introduction_word_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """
        Validate word count for Introduction section.
        """
            ...
            return FunctionResult(
                ...
                data={
                    ...
                    "issues": [{
                        ...
                        "severity": "low",
                        "message": "Introduction section not found",
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for introduction word_count: {e}")
        ...
        if not analysis["meets_target"]:
            if analysis["word_count"] < self.optimal_min:
                issues.append({
                    ...
                    "severity": "low",
                    "message": f"Introduction section word count ({analysis['word_count']}) is below optimal range ({self.optimal_min}-{self.optimal_max} words, Q25-Q75 from successful motions)",
                    "suggestion": f"Increase Introduction section to at least {self.optimal_min} words"
                })
                recommendation = {
                    "type": "increase_section_length",
                    "priority": "low",
                    "action": f"Increase Introduction section from {analysis['word_count']} to at least {self.optimal_min} words",
                    "rationale": "Optimal word count range for Introduction section"
                }
                ...
                issues.append({
                    ...
                    "severity": "low",
                    "message": f"Introduction section word count ({analysis['word_count']}) exceeds optimal range ({self.optimal_min}-{self.optimal_max} words, Q25-Q75 from successful motions)",
                    "suggestion": f"Consider condensing Introduction section to {self.optimal_max} words or less"
                })
        else:
            recommendations.append({
                "type": "section_length_optimal",
                "message": f"Introduction section word count is optimal ({analysis['word_count']} words, range: {self.optimal_min}-{self.optimal_max})"
            })
        ...
        return FunctionResult(
            ...
            message="Introduction section word count validation complete"
        )
