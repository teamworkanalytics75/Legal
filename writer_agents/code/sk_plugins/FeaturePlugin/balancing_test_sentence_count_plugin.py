#!/usr/bin/env python3
"""
Balancing Test Section Sentence Count Plugin - Enforces sentences per paragraph for Balancing Test section.

Validates that paragraphs in the Balancing Test section have optimal sentence counts based on analysis.
"""
...
class BalancingTestSentenceCountPlugin(BaseFeaturePlugin):
    """Plugin to validate sentences per paragraph for Balancing Test section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "balancing_test_sentence_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("BalancingTestSentenceCountPlugin initialized")

        loader = get_loader()
        sentences_range = loader.get_sentences_per_paragraph_range('balancing_test')
    ...
    async def validate_balancing_test_sentence_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate sentences per paragraph for Balancing Test section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for balancing_test sentence_count: {e}")
        
        if not analysis["meets_target"]:
            issues.append({
                "type": "suboptimal_sentence_count",
                "severity": "low",
                "message": f"Balancing Test section has {analysis['paragraphs_in_range']}/{analysis['total_paragraphs']} paragraphs with optimal sentence count ({self.optimal_min}-{self.optimal_max} sentences)",
            ...
            recommendation = {
                ...
                "rationale": "Optimal sentence count per paragraph for Balancing Test section"
            }
            ...
            recommendations.append({
                ...
                "message": f"Balancing Test section sentence count per paragraph is optimal (avg: {analysis['avg_sentences_per_paragraph']:.1f})"
            })
        ...
        return FunctionResult(
            ...
            message="Balancing Test section sentence count validation complete"
        )
