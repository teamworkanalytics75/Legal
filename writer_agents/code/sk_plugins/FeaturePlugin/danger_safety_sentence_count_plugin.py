#!/usr/bin/env python3
"""
Danger/Safety Section Sentence Count Plugin - Enforces sentences per paragraph for Danger/Safety section.

Validates that paragraphs in the Danger/Safety section have optimal sentence counts based on analysis.
"""
...
class DangerSafetySentenceCountPlugin(BaseFeaturePlugin):
    """Plugin to validate sentences per paragraph for Danger/Safety section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "danger_safety_sentence_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("DangerSafetySentenceCountPlugin initialized")

        loader = get_loader()
        sentences_range = loader.get_sentences_per_paragraph_range('danger_safety')
    ...
    async def validate_danger_safety_sentence_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate sentences per paragraph for Danger/Safety section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for danger_safety sentence_count: {e}")
        
        if not analysis["meets_target"]:
            issues.append({
                "type": "suboptimal_sentence_count",
                "severity": "low",
                "message": f"Danger/Safety section has {analysis['paragraphs_in_range']}/{analysis['total_paragraphs']} paragraphs with optimal sentence count ({self.optimal_min}-{self.optimal_max} sentences)",
            ...
            recommendation = {
                ...
                "rationale": "Optimal sentence count per paragraph for Danger/Safety section"
            }
            ...
            recommendations.append({
                ...
                "message": f"Danger/Safety section sentence count per paragraph is optimal (avg: {analysis['avg_sentences_per_paragraph']:.1f})"
            })
        ...
        return FunctionResult(
            ...
            message="Danger/Safety section sentence count validation complete"
        )
