#!/usr/bin/env python3
"""
Introduction Section Sentence Count Plugin - Enforces sentences per paragraph for Introduction section.

Validates that paragraphs in the Introduction section have optimal sentence counts based on analysis.
"""
...
class IntroductionSentenceCountPlugin(BaseFeaturePlugin):
    """Plugin to validate sentences per paragraph for Introduction section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "introduction_sentence_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("IntroductionSentenceCountPlugin initialized")

        loader = get_loader()
        sentences_range = loader.get_sentences_per_paragraph_range('introduction')
    ...
    async def validate_introduction_sentence_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate sentences per paragraph for Introduction section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for introduction sentence_count: {e}")
        
        if not analysis["meets_target"]:
            issues.append({
                "type": "suboptimal_sentence_count",
                "severity": "low",
                "message": f"Introduction section has {analysis['paragraphs_in_range']}/{analysis['total_paragraphs']} paragraphs with optimal sentence count ({self.optimal_min}-{self.optimal_max} sentences)",
            ...
            recommendations.append({
                ...
                "message": f"Introduction section sentence count per paragraph is optimal (avg: {analysis['avg_sentences_per_paragraph']:.1f})"
            })
        
        # Add corpus examples to recommendations if available
        if corpus_examples and not analysis["meets_target"]:
            for rec in recommendations:
                if rec.get("type") == "suboptimal_sentence_count":
                    rec["corpus_examples"] = [
                        {
        ...
        return FunctionResult(
            ...
            message="Introduction section sentence count validation complete"
        )
