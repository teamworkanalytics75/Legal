#!/usr/bin/env python3
"""
Factual Background Section Sentence Count Plugin - Enforces sentences per paragraph for Factual Background section.

Validates that paragraphs in the Factual Background section have optimal sentence counts based on analysis.
"""
...
class FactualBackgroundSentenceCountPlugin(BaseFeaturePlugin):
    """Plugin to validate sentences per paragraph for Factual Background section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "factual_background_sentence_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("FactualBackgroundSentenceCountPlugin initialized")

        loader = get_loader()
        sentences_range = loader.get_sentences_per_paragraph_range('factual_background')
    ...
    async def validate_factual_background_sentence_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate sentences per paragraph for Factual Background section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for factual_background sentence_count: {e}")
        
        if not analysis["meets_target"]:
            issues.append({
                "type": "suboptimal_sentence_count",
                "severity": "low",
                "message": f"Factual Background section has {analysis['paragraphs_in_range']}/{analysis['total_paragraphs']} paragraphs with optimal sentence count ({self.optimal_min}-{self.optimal_max} sentences)",
            ...
            recommendation = {
                ...
                "rationale": "Optimal sentence count per paragraph for Factual Background section"
            }
            ...
            recommendations.append({
                ...
                "message": f"Factual Background section sentence count per paragraph is optimal (avg: {analysis['avg_sentences_per_paragraph']:.1f})"
            })
        ...
        return FunctionResult(
            ...
            message="Factual Background section sentence count validation complete"
        )
