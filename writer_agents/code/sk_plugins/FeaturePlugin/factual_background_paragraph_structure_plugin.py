#!/usr/bin/env python3
"""
Factual Background Section Paragraph Structure Plugin - Enforces paragraph structure for Factual Background section.
...
class FactualBackgroundParagraphStructurePlugin(BaseFeaturePlugin):
    """Plugin to validate paragraph structure for Factual Background section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "factual_background_paragraph_structure", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("FactualBackgroundParagraphStructurePlugin initialized")

        loader = get_loader()
        para_count_range = loader.get_paragraph_count_range('factual_background')
        avg_words_range = loader.get_avg_words_per_paragraph_range('factual_background')
        
        if para_count_range:
            self.optimal_para_min = int(para_count_range[0])
            self.optimal_para_max = int(para_count_range[1])
        else:
            self.optimal_para_min = 13
            self.optimal_para_max = 95
    ...
    async def validate_factual_background_paragraph_structure(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate paragraph structure for Factual Background section."""
        text = document.get_full_text()
        section_text = extract_section_text(text, 'factual_background')
        
        if not section_text:
            return FunctionResult(success=False, data={"section_found": False, "issues": [{"type": "missing_section", "severity": "high", "message": "Factual Background section not found"}]}, message="Factual Background section not found")
        ...
        if not analysis["meets_target"]:
            try:
                ...
                corpus_examples = await find_section_examples_from_corpus(
                    ...
                    section_name='factual_background',
                    feature_type='paragraph_structure',
                    optimal_range=optimal_range,
                    limit=5
                )
            except Exception as e:
                logger.debug(f"Corpus search failed for factual_background paragraph_structure: {e}")
        
        if not analysis["meets_para_target"]:
            if analysis["paragraph_count"] < self.optimal_para_min:
                issues.append({"type": "insufficient_paragraphs", "severity": "medium", "message": f"Factual Background section has {analysis['paragraph_count']} paragraphs, below optimal range ({self.optimal_para_min}-{self.optimal_para_max})"})
            elif analysis["paragraph_count"] > self.optimal_para_max:
                issues.append({"type": "excessive_paragraphs", "severity": "low", "message": f"Factual Background section has {analysis['paragraph_count']} paragraphs, above optimal range ({self.optimal_para_min}-{self.optimal_para_max})"})
        
        if not analysis["meets_avg_target"]:
            issues.append({"type": "suboptimal_avg_words", "severity": "low", "message": f"Factual Background section average words per paragraph ({analysis['avg_words_per_paragraph']:.0f}) differs from optimal ({self.optimal_avg_words:.0f})"})
        
        if analysis["meets_target"]:
            recommendations.append({"type": "paragraph_structure_optimal", "message": f"Factual Background section paragraph structure is optimal"})
        else:
        ...
        return FunctionResult(
            ...
            message="Factual Background section paragraph structure validation complete"
        )
