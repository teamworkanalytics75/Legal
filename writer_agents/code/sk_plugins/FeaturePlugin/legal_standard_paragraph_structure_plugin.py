#!/usr/bin/env python3
"""
Legal Standard Section Paragraph Structure Plugin - Enforces paragraph structure for Legal Standard section.
"""
...
class LegalStandardParagraphStructurePlugin(BaseFeaturePlugin):
    """Plugin to validate paragraph structure for Legal Standard section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "legal_standard_paragraph_structure", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("LegalStandardParagraphStructurePlugin initialized")

        # Load optimal ranges from analysis data
        loader = get_loader()
        para_count_range = loader.get_paragraph_count_range('legal_standard')
        avg_words_range = loader.get_avg_words_per_paragraph_range('legal_standard')
        
        if para_count_range:
            self.optimal_para_min = int(para_count_range[0])
            self.optimal_para_max = int(para_count_range[1])
        else:
            self.optimal_para_min = 11
            self.optimal_para_max = 165
    ...
    def _analyze_paragraph_structure(self, section_text: str) -> Dict[str, Any]:
        ...
        meets_avg_target = abs(avg_words - self.optimal_avg_words) / self.optimal_avg_words < 0.3  # Within 30%
    ...
    async def validate_legal_standard_paragraph_structure(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate paragraph structure for Legal Standard section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for legal_standard paragraph_structure: {e}")
        
        if not analysis["meets_para_target"]:
            if analysis["paragraph_count"] < self.optimal_para_min:
                issues.append({"type": "insufficient_paragraphs", "severity": "medium", "message": f"Legal Standard section has {analysis['paragraph_count']} paragraphs, below optimal range ({self.optimal_para_min}-{self.optimal_para_max})"})
            elif analysis["paragraph_count"] > self.optimal_para_max:
                issues.append({"type": "excessive_paragraphs", "severity": "low", "message": f"Legal Standard section has {analysis['paragraph_count']} paragraphs, above optimal range ({self.optimal_para_min}-{self.optimal_para_max})"})
        
        if not analysis["meets_avg_target"]:
            issues.append({"type": "suboptimal_avg_words", "severity": "low", "message": f"Legal Standard section average words per paragraph ({analysis['avg_words_per_paragraph']:.0f}) differs from optimal ({self.optimal_avg_words})"})
        
        if analysis["meets_target"]:
            recommendations.append({"type": "paragraph_structure_optimal", "message": f"Legal Standard section paragraph structure is optimal"})
        else:
        ...
        return FunctionResult(
            ...
            message="Legal Standard section paragraph structure validation complete"
        )
