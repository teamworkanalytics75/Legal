#!/usr/bin/env python3
"""
Legal Standard Section Sentence Count Plugin - Enforces sentences per paragraph for Legal Standard section.

Validates that paragraphs in the Legal Standard section have optimal sentence counts based on analysis.
"""
...
class LegalStandardSentenceCountPlugin(BaseFeaturePlugin):
    """Plugin to validate sentences per paragraph for Legal Standard section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "legal_standard_sentence_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("LegalStandardSentenceCountPlugin initialized")

        # Load optimal ranges from analysis data
        loader = get_loader()
        sentences_range = loader.get_sentences_per_paragraph_range('legal_standard')
    ...
    def _analyze_sentence_count(self, section_text: str) -> Dict[str, Any]:
        ...
        # Check if most paragraphs are within range
        paragraphs_in_range = sum(1 for count in paragraph_sentence_counts if self.optimal_min <= count <= self.optimal_max)
        meets_target = (paragraphs_in_range / len(paragraph_sentence_counts)) >= 0.7  # 70% within range
    ...
    async def validate_legal_standard_sentence_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate sentences per paragraph for Legal Standard section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for legal_standard sentence_count: {e}")
        
        if not analysis["meets_target"]:
            issues.append({
                "type": "suboptimal_sentence_count",
                "severity": "low",
                "message": f"Legal Standard section has {analysis['paragraphs_in_range']}/{analysis['total_paragraphs']} paragraphs with optimal sentence count ({self.optimal_min}-{self.optimal_max} sentences)",
            ...
            recommendation = {
                ...
                "rationale": "Optimal sentence count per paragraph for Legal Standard section"
            }
            ...
            recommendations.append({
                ...
                "message": f"Legal Standard section sentence count per paragraph is optimal (avg: {analysis['avg_sentences_per_paragraph']:.1f})"
            })
        ...
        return FunctionResult(
            ...
            message="Legal Standard section sentence count validation complete"
        )
