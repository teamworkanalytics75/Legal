#!/usr/bin/env python3
"""
Legal Standard Section Word Count Plugin - Enforces word count for Legal Standard section.

Validates that the Legal Standard section has optimal word count based on CatBoost analysis.
...
class LegalStandardWordCountPlugin(BaseFeaturePlugin):
    """Plugin to validate word count for Legal Standard section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "legal_standard_word_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("LegalStandardWordCountPlugin initialized")

        # Load optimal word count range from analysis data
        loader = get_loader()
        word_count_range = loader.get_word_count_range('legal_standard')
        ...
        if word_count_range:
            ...
            # Fallback to rules or defaults
            targets = self.rules.get("targets", {})
            self.optimal_min = targets.get("min_word_count", 762)
            self.optimal_max = targets.get("max_word_count", 4249)
    ...
    def _analyze_section_word_count(self, section_text: str) -> Dict[str, Any]:
        """Analyze word count in Legal Standard section."""
        if not section_text:
            return {
                "word_count": 0,
                "meets_target": False,
    ...
    async def _find_corpus_examples(self) -> List[Dict[str, Any]]:
        """Find corpus examples of successful Legal Standard sections with optimal word count."""
        try:
            ...
            logger.debug(f"Corpus search failed for legal_standard word_count: {e}")
            return []

    async def validate_legal_standard_word_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """
        Validate word count for Legal Standard section.
        """
        ...
        if not analysis["meets_target"]:
            corpus_examples = await self._find_corpus_examples()
        ...
        if not analysis["meets_target"]:
            if analysis["word_count"] < self.optimal_min:
                issues.append({
                    ...
                    "severity": "medium",
                    "message": f"Legal Standard section word count ({analysis['word_count']}) is below optimal range ({self.optimal_min}-{self.optimal_max} words, Q25-Q75 from successful motions)",
                    "suggestion": f"Increase Legal Standard section to at least {self.optimal_min} words"
                })
                recommendation = {
                    "type": "increase_section_length",
                    "priority": "medium",
                    "action": f"Increase Legal Standard section from {analysis['word_count']} to at least {self.optimal_min} words",
                    "rationale": "Optimal word count range for Legal Standard section"
                }
                ...
                issues.append({
                    ...
                    "severity": "low",
                    "message": f"Legal Standard section word count ({analysis['word_count']}) exceeds optimal range ({self.optimal_min}-{self.optimal_max})",
                    "suggestion": f"Consider condensing Legal Standard section to {self.optimal_max} words or less"
                })
        else:
            recommendations.append({
                "type": "section_length_optimal",
                "message": f"Legal Standard section word count is optimal ({analysis['word_count']} words, range: {self.optimal_min}-{self.optimal_max})"
            })
        ...
        return FunctionResult(
            ...
            message="Legal Standard section word count validation complete"
        )
