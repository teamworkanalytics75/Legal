#!/usr/bin/env python3
"""
Privacy Harm Section Word Count Plugin - Enforces word count for Privacy Harm section.
...
class PrivacyHarmWordCountPlugin(BaseFeaturePlugin):
    """Plugin to validate word count for Privacy Harm section."""

    def __init__(self, kernel: Kernel, chroma_store, rules_dir: Path, memory_store=None, **kwargs):
        super().__init__(kernel, "privacy_harm_word_count", chroma_store, rules_dir, memory_store=memory_store, **kwargs)
        logger.info("PrivacyHarmWordCountPlugin initialized")

        # Load optimal word count range from analysis data
        loader = get_loader()
        word_count_range = loader.get_word_count_range('privacy_harm')
        ...
        if word_count_range:
            ...
            self.optimal_min = targets.get("min_word_count", 347)
            self.optimal_max = targets.get("max_word_count", 3000)
    ...
    def _analyze_section_word_count(self, section_text: str) -> Dict[str, Any]:
        """Analyze word count in Privacy Harm section."""
        if not section_text:
    ...
    async def validate_privacy_harm_word_count(self, document: "DocumentStructure", context: Dict[str, Any] = None) -> FunctionResult:
        """Validate word count for Privacy Harm section."""
        ...
        if not analysis["meets_target"]:
            try:
                ...
                logger.debug(f"Corpus search failed for privacy_harm word_count: {e}")
        ...
        if not analysis["meets_target"]:
            if analysis["word_count"] < self.optimal_min:
                issues.append({
                    ...
                    "severity": "medium",
                    "message": f"Privacy Harm section word count ({analysis['word_count']}) is below optimal range ({self.optimal_min}-{self.optimal_max} words, Q25-Q75 from successful motions)",
                    "suggestion": f"Increase Privacy Harm section to at least {self.optimal_min} words"
                })
                recommendation = {
                    "type": "increase_section_length",
                    "priority": "medium",
                    "action": f"Increase Privacy Harm section from {analysis['word_count']} to at least {self.optimal_min} words",
                    "rationale": "Optimal word count range for Privacy Harm section"
                }
                ...
                issues.append({
                    ...
                    "severity": "low",
                    "message": f"Privacy Harm section word count ({analysis['word_count']}) exceeds optimal range ({self.optimal_min}-{self.optimal_max})",
                    "suggestion": f"Consider condensing Privacy Harm section to {self.optimal_max} words or less"
                })
        else:
            recommendations.append({
                "type": "section_length_optimal",
                "message": f"Privacy Harm section word count is optimal ({analysis['word_count']} words, range: {self.optimal_min}-{self.optimal_max})"
            })
        ...
        return FunctionResult(
            ...
            message="Privacy Harm section word count validation complete"
        )
