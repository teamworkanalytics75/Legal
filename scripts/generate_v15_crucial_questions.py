#!/usr/bin/env python3
"""Generate crucial clarification questions for v15 dataset."""
...
OUTPUT_CSV = Path("case_law_data/crucial_questions_v15.csv")
OUTPUT_TXT = Path("case_law_data/crucial_questions_v15.txt")
REPORT_PATH = Path("reports/analysis_outputs/crucial_questions_v15_report.md")
...
def detect_crucial_issues(fact: dict) -> list[dict]:
    ...
    if is_high_salience:
        ...
        elif actorrole != subject and 'harvard' in prop.lower() and 'prc' in prop.lower():
            # Potential misclassification
            issues.append({
                'type': 'actor_subject_mismatch',
                'priority': 'medium',
                'question': f"Fact {factid} (Salience: {salience:.2f}) mentions both Harvard and PRC/China. Is the ActorRole/Subject correctly assigned?",
                'context': prop[:200],
                'factid': factid,
                'salience': salience,
            })
    
    # Issue 4: Missing causal connection in high-salience facts
    if is_high_salience and 'harvard' in prop.lower() and ('prc' in prop.lower() or 'china' in prop.lower()):
        if 'caused' not in prop.lower() and 'led to' not in prop.lower() and 'resulted in' not in prop.lower() and 'foreseeable' not in prop.lower():
            issues.append({
                'type': 'missing_causal_connection',
                'priority': 'critical',
                'question': f"Fact {factid} (Salience: {salience:.2f}) mentions both Harvard and PRC/China but doesn't explicitly state the causal connection. How does Harvard's action relate to PRC outcomes?",
                'context': prop[:200],
                'factid': factid,
                'salience': salience,
            })
...
def main():
    """Main execution."""
    print("="*80)
    print("GENERATING CRUCIAL QUESTIONS FOR V15")
    print("="*80)
    ...
    lines = []
    lines.append("="*80)
    lines.append("CRUCIAL CLARIFICATION QUESTIONS - V15")
    lines.append("="*80)
    lines.append("")
    ...
    print("\n7. Writing report...")
    report = f"""# Crucial Questions Report (V15)
...
- **CSV**: `crucial_questions_v15.csv` (all {len(prioritized)} questions)
- **TXT**: `crucial_questions_v15.txt` (top 20 questions)

## Next Steps

1. Review the top 20 questions in the TXT file
2. Answer the critical and high-priority questions first
3. These answers will help improve the dataset's accuracy and completeness
"""
    ...
    print(f"\n{'='*80}")
    print("✅ COMPLETE! Crucial questions generated")
    print(f"{'='*80}")
    print(f"\nSummary:")
